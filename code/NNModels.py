...
class Linear:
    def __init__(self, N, Nfeat, minibatch_size, learning_rate):
        ...
        self.checkpoint_dir = os.path.join(my_path, "../checkpoints/ckpts_linear_N%d_mb%d_fe_%d" % (N, minibatch_size, Nfeat))
...
class SingleFull: # recommend 9x9, mbs=1000, adam, lr=0.003
    def __init__(self, N, Nfeat, minibatch_size, learning_rate):
        self.checkpoint_dir = os.path.join(my_path, "../checkpoints/ckpts_single_full_N%d_mb%d_fe_%d" % (N, minibatch_size, Nfeat))
        self.learning_rate = learning_rate
    def inference(self, feature_planes, N, Nfeat):
        Nhidden = 1024
        flat_features = tf.reshape(feature_planes, [-1, N*N*Nfeat])
...
class Conv3Full: # recommend 9x9, mbs=1000, adam, lr=0.003
    def __init__(self, N, Nfeat, minibatch_size, learning_rate):
        self.checkpoint_dir = os.path.join(my_path, "../checkpoints/ckpts_conv3_full_N%d_mb%d_fe_%d" % (N, minibatch_size, Nfeat))
        self.learning_rate = learning_rate
    def inference(self, feature_planes, N, Nfeat):
        NK = 32
        Nhidden = 1024
...
class Conv4Full:
    def __init__(self, N, Nfeat, minibatch_size, learning_rate):
        self.checkpoint_dir = os.path.join(my_path, "../checkpoints/ckpts_conv4_full_N%d_mb%d_fe_%d" % (N, minibatch_size, Nfeat))
        self.learning_rate = learning_rate
    def inference(self, feature_planes, N, Nfeat):
        NK = 64
        Nhidden = 1024
...
class Conv5Full:
    def __init__(self, N, Nfeat, minibatch_size, learning_rate):
        self.checkpoint_dir = os.path.join(my_path, "../checkpoints/ckpts_conv5_full_N%d_mb%d_fe_%d" % (N, minibatch_size, Nfeat))
        self.learning_rate = learning_rate
    def inference(self, feature_planes, N, Nfeat):
        NK = 128
        Nhidden = 1024
...
class Conv8:
    def __init__(self, N, Nfeat, minibatch_size=1000, learning_rate=0.0003):
        self.checkpoint_dir = os.path.join(my_path, "../checkpoints/ckpts_conv8_N%d_mb%d_fe%d" % (N, minibatch_size, Nfeat))
        self.learning_rate = learning_rate
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
...
class Conv8Full:
    def __init__(self, N, Nfeat, minibatch_size, learning_rate):
        self.checkpoint_dir = os.path.join(my_path, "../train_dirs/ckpts_conv8_full_N%d_mb%d_fe%d" % (N, minibatch_size, Nfeat))
        self.learning_rate = learning_rate
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
...
class Conv12: # AlphaGo architecture
    def __init__(self, N, Nfeat, minibatch_size=1000, learning_rate=0.0003):
        self.checkpoint_dir = os.path.join(my_path, "../checkpoints/ckpts_conv12_N%d_mb%d_fe%d" % (N, minibatch_size, Nfeat))
        self.learning_rate = learning_rate
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
...
class MaddisonMinimal:
    def __init__(self, N, Nfeat, minibatch_size=1000, learning_rate=0.0003):
        self.checkpoint_dir = os.path.join(my_path, "../checkpoints/maddison_minimal_N%d_mb%d_fe%d" % (N, minibatch_size, Nfeat))
        self.learning_rate = learning_rate
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
...
class Conv6PosDep:
    def __init__(self, N, Nfeat):
        self.checkpoint_dir = os.path.join(my_path, "../train_dirs/conv6posdep_N%d_fe%d" % (N, Nfeat))
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
        NK = 128
...
class Conv8PosDep:
    def __init__(self, N, Nfeat):
        sself.checkpoint_dir = os.path.join(my_path, "../train_dirs/conv8posdep_N%d_fe%d" % (N, Nfeat))
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
        NK = 192
...
class Conv10PosDep:
    def __init__(self, N, Nfeat):
        self.checkpoint_dir = os.path.join(my_path, "../train_dirs/conv10posdep_N%d_fe%d" % (N, Nfeat))
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
        NK = 192
...
class Conv5PosDepELU:
    def __init__(self, N, Nfeat):
        self.train_dir = os.path.join(my_path, "../checkpoints/conv5posdepELU_N%d_fe%d" % (N, Nfeat))
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
        NK = 20
        NKfirst = 20
        conv1 = ELU_conv_pos_dep_bias(feature_planes, 3, Nfeat, NKfirst, N, 'conv1')
        conv2 = ELU_conv_pos_dep_bias(conv1, 3, NKfirst, NK, N, 'conv2')
        conv3 = ELU_conv_pos_dep_bias(conv2, 3, NK, NK, N, 'conv3')
        conv4 = ELU_conv_pos_dep_bias(conv3, 3, NK, NK, N, 'conv4')
        conv5 = conv_pos_dep_bias(conv4, 3, NK, 2, N, 'conv5')
        logits = tf.reshape(conv5, [-1, N*N*2])
        return logits

class Conv12PosDepELUBig:
    def __init__(self, N, Nfeat):
        self.checkpoint_dir = os.path.join(my_path, "../train_dirs/conv12posdepELUbig_N%d_fe%d" % (N, Nfeat))
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
        NK = 512
...
class Conv16PosDepELU:
    def __init__(self, N, Nfeat):
        self.checkpoint_dir = os.path.join(my_path, "../train_dirs/conv16posdepELU_N%d_fe%d" % (N, Nfeat))
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
        NK = 256
...
class Conv4PosDepELU:
    def __init__(self, N, Nfeat):
        self.checkpoint_dir = os.path.join(my_path, "../train_dirs/conv4posdepELU_N%d_fe%d" % (N, Nfeat))
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
        NK = 48
...
class Conv12PosDep:
    def __init__(self, N, Nfeat):
        self.checkpoint_dir = os.path.join(my_path, "../train_dirs/conv12posdep_N%d_fe%d" % (N, Nfeat))
        self.N = N
        self.Nfeat = Nfeat
    def inference(self, feature_planes, N, Nfeat):
        NK = 192
...
if __name__ == "__main__":
    test = Conv12PosDepELU(5, 10)
    feature_planes = tf.placeholder(tf.float32, shape=[None, 5, 5, 10])

    x = test.inference(feature_planes, 5, 10)
